...
pause
close
disp('实验2  用Jacobi法求对称矩阵的特征值及特征向量')
tic
tezhjcobi
t(2)=toc;
disp('---按任意键继续下一题---')
...
legend('扰动前的解','扰动后的解',0);
hold off;
disp(['对',num2str(Numb),'次项加扰动',num2str(ess),'得到的全部根为：',]);
for i=1:length(root)
    if imag(root(i))<0
...
disp(['偏差最大的根是第',num2str(zdxb),'个；']);
disp(['扰动前为：',num2str(zdxb)]);
disp(['扰动后为：',num2str(root(21-zdxb))]);
disp(['相应的条件数为：',num2str(cpshu(zdxb))]);
disp(['实际误差为：',num2str(abs(root(21-zdxb)-an(21-zdxb)))]);
function tezhjcobi
...
for i=1:3
  ...
  fprintf('迭代次数:%2d\n',k);
  for j=1:size(a,1)
     disp(['第',num2str(j),'个特征值为:',num2str(lm(j))]);
     disp(['相应的特征向量为:',num2str(vecter(:,j)')]);
  end
end
...
disp(['要计算的',num2str(length(t)),'个节点处的值为:'])
for i=1:length(t)
    disp(['S(',num2str(t(i)),')=',num2str(St(i))]);
end
disp(['要计算的',num2str(length(t)),'个节点处的一阶导数值为:'])
for i=1:length(t)
    disp(['dS(',num2str(t(i)),')=',num2str(Std1(i))]);
end
disp(['要计算的',num2str(length(t)),'个节点处的二阶导数值为:'])
for i=1:length(t)
...
y(1)=f(1)/alf(1);
for i=2:n
    y(i)=(f(i)-a(i-1)*y(i-1))/alf(i);%追的过程
end
x(n)=y(n);
for i=n-1:-1:1
    x(i)=y(i)-bita(i)*x(i+1);%赶的过程
end
